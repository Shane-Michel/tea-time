...
LOG_DIR="$RELEASE_DIR/api/storage/logs"
...
4. Make sure `api/storage/logs` is writable by the web server user so client/server errors can be recorded.
5. The `public` assets already include `robots.txt`, `sitemap.xml`, and Open Graph images. Regenerate with `npm run generate:seo` before rebuilding if routes change.
DOC
